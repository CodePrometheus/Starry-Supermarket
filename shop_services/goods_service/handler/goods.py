...
from shop_services.goods_service.model.models import Brands, Category, GoodsCategoryBrand, Goods, Banner
from shop_services.goods_service.proto import goods_pb2_grpc, goods_pb2
...
class GoodsServicer(goods_pb2_grpc.GoodsServicer):
    ...
    def GoodsList(self, request: goods_pb2.GoodsFilterRequest, context):
        ...
        if request.topCategory:
            # 通过category来查询商品， category可能是一级、二级或者三级分类
            try:
                ids = []
        ...
        for good in goods:
            rsp.data.append(convert_model_to_message(good))
    ...
    def GetSubCategory(self, request: goods_pb2.CategoryListRequest, context):
        ...
        category_list_rsp.total = categories.count()
        for category in categories:
            category_rsp = goods_pb2.CategoryInfoResponse()
            category_rsp.id = category.id
            category_rsp.name = category.name
